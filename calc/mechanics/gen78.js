...
function calculateSMSS(gen, attacker, defender, move, field) {
    ...
    var isCritical = !defender.hasAbility('Battle Armor', 'Shell Armor') &&
        (move.isCrit || (attacker.hasAbility('Merciless') && (defender.hasStatus('psn', 'tox') || field.hasTerrain('Corrosive', 'Corrosive Mist', 'Murkwater', 'Wasteland')))) &&
        move.timesUsed === 1;
    var type = move.type;
    if (move.named('Weather Ball')) {
        ...
        type =
            field.hasTerrain('Ashen Beach', 'Inverse', 'Misty', 'Psychic', 'Dance') ? 'Psychic'
                : field.hasTerrain('Big Top', 'Cloud') ? 'Flying'
                    : field.hasTerrain('Burning', 'Superheated') ? 'Fire'
                        : field.hasTerrain('Cave', 'Chess Board', 'Crystal Fire', 'Crystal Water', 'Crystal Grass', 'Crystal Psychic', 'Mountain') ? 'Rock'
                            : field.hasTerrain('Corrosive', 'Corrosive Mist', 'Murkwater', 'Wasteland') ? 'Poison'
                                : field.hasTerrain('Dark Crystal', 'Darkness 1', 'Darkness 2', 'Darkness 3') ? 'Dark'
                                        ...
                                        : field.hasTerrain('Dragon\'s Den', 'New World') ? 'Dragon'
                                            : field.hasTerrain('Electric', 'Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2') ? 'Electric'
                                                : field.hasTerrain('Factory', 'Mirror', 'Fairy Tale') ? 'Steel'
                                                    : field.hasTerrain('Rocky', 'Crowd') ? 'Fighting'
    ...
    if (field.hasTerrain('Fairy Tale') && move.named('Cut', 'Sacred Sword', 'Secret Sword', 'Slash')) {
        type = 'Steel';
    }
    if (field.hasTerrain('Murkwater') && move.named('Mud Bomb', 'Mud Shot', 'Mud-Slap', 'Thousand Waves')) {
        type = 'Water';
    }
    if (field.hasTerrain('Starlight') && move.named('Solar Beam', 'Solar Blade')) {
        type = 'Fairy';
    }
    ...
    if ((attacker.hasAbility('Triage') && move.drain) || (attacker.hasAbility('Gale Wings') &&
        move.hasType('Flying') &&
    ...
    if (typeEffectiveness === 0 && (move.named('Thousand Arrows') || (move.hasType('Ground') && field.hasTerrain('Cave')))) {
        typeEffectiveness = 1;
    }
    ...
    if (field.hasTerrain('Dragon\'s Den') && defender.hasAbility('Multiscale') &&
    ...
    if (!noWeatherBoost && ((field.hasWeather('Sun', 'Harsh Sunshine') && move.hasType('Fire')) || (field.hasWeather('Rain', 'Heavy Rain') && move.hasType('Water')))) {
        baseDamage = (0, util_2.pokeRound)((0, util_2.OF32)(baseDamage * 6144) / 4096);
        desc.weather = field.weather;
    }
    else if (!noWeatherBoost && ((field.hasWeather('Sun') && move.hasType('Water')) || (field.hasWeather('Rain') && move.hasType('Fire')))) {
...
function calculateBasePowerSMSS(gen, attacker, defender, move, field, hasAteAbilityTypeChange, desc) {
    ...
    switch (move.name) {
        ...
        case 'Low Kick':
            ...
            basePower = we >= 200 ? 120 : we >= 100 ? 100 : we >= 50 ? 80 : we >= 25 ? 60 : we >= 10 ? 40 : 20;
            desc.moveBP = basePower;
            break;
        case 'Grass Knot':
    ...
    switch (field.terrain) {
        ...
        case 'Chess Board':
            ...
            break;
		case 'Cloud':
			if (move.named('Fairy Wind', 'Icy Wind', 'Ominous Wind', 'Razor Wind', 'Silver Wind', 'Steel Wing', 'Twister')) {
				basePower *= 1.2;
			}
			...
			else if (move.hasType('Rock')) {
				basePower *= 0.75
			}
			else if (move.hasType('Ground')) {
			basePower *= 0.0;
			}
			desc.moveBP = basePower;
            desc.terrain = field.terrain;
        ...
        case 'Corrosive Mist':
            ...
            break;
        case 'Crowd':
			if (move.named('Acrobatics', 'Aerial Ace', 'Beat Up', 'Blaze Kick', 'Body Press', 'Body Slam', 'Brave Bird', 'Breakneck Blitz', 'Brutal Swing', 'Comet Punch', 'Dizzy Punch', 'Double Hit', 'Double-Edge', 'Fire Punch', 'Giga Impact', 'Head Smash', 'Headbutt', 'Heavy Slam', 'Ice Punch', 'Iron Head', 'Knock Off', 'Mega Kick', 'Mega Punch', 'Meteor Mash', 'Play Rough', 'Punishment', 'Shadow Punch', 'Skull Bash', 'Slam', 'Spirit Break', 'Steel Wing', 'Strength', 'Sucker Punch', 'Thunder Punch', 'Triple Axel', 'Trop Kick', 'Zen Headbutt')) {
				basePower *= 1.2;
			}
        ...
        case 'Crystal Psychic':
            ...
            break;
        case 'Dance':
			if (move.named('Aura Sphere', 'Boomburst', 'Bug Buzz', 'Chatter', 'Clanging Scales', 'Dazzling Gleam', 'Disarming Voice', 'Echoed Voice', 'Eerie Spell', 'Flash Cannon', 'Hyper Voice', 'Mystical Fire', 'Overdrive', 'Relic Song', 'Round', 'Signal Beam', 'Snarl', 'Sparkling Aria', 'Uproar')) {
				basePower *= 1.2;
			}
			else if (move.named('Fiery Dance', 'Petal Dance', 'Revelation Dance')) {
				basePower *= 1.4;
			}
			...
			break;
		case 'Darkness 1':
			if (move.named('Astral Barrage', 'Black Hole Eclipse', 'Dark Pulse', 'False Surrender', 'Moongeist Beam', 'Never-Ending Nightmare', 'Night Daze', 'Night Slash', 'Ominous Wind', 'Phantom Froce', 'Shadow Ball', 'Shadow Bone', 'Shadow Claw', 'Shadow Force', 'Shadow Punch', 'Shadow Sneak', 'Soul-Stealing 7-Star Strike', 'Spectral Thief', 'Spirit Shackle', 'Sucker Punch', 'Throat Chop')) {
				basePower *= 1.1;
			}
			else if (move.named('Aurora Beam', 'Dazzling Gleam', 'Doom Desire', 'Flash Cannon', 'Luster Purge', 'Mirror Shot', 'Photon Geyser', 'Power Gem', 'Signal Beam', 'Strange Steam', 'Techno Blast')) {
				basePower *= 0.9;
			}
        ...
        case 'Superheated':
            ...
            if (move.hasType('Water') && !move.named('Scald', 'Steam Eruption', 'Muddy Water', 'Sparkling Aria', 'Surf', 'Water Pledge', 'Water Spout', 'Hydro Vortex', 'Oceanic Operetta')) {
                basePower *= 0.9;
            }
            else if (move.hasType('Fire')) {
                basePower *= 1.1;
        ...
        case 'Underwater':
            ...
            if (move.category === 'Physical' && (!move.hasType('Water') || !attacker.hasAbility('Steelworker'))) {
                basePower *= 0.5;
            }
            desc.moveBP = basePower;
            desc.terrain = field.terrain;
...
function calculateBPModsSMSS(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    ...
    else if (move.named('Solar Beam', 'Solar Blade') && field.hasWeather('Rain', 'Heavy Rain', 'Sand', 'Hail')) {
        bpMods.push(2048);
        desc.moveBP = basePower / 2;
        desc.weather = field.weather;
    }
    ...
    if ((attacker.hasAbility('Flare Boost') &&
        (attacker.hasStatus('brn') || field.hasTerrain('Burning')) && move.category === 'Special') ||
        (attacker.hasAbility('Toxic Boost') &&
            (attacker.hasStatus('psn', 'tox') || field.hasTerrain('Corrosive', 'Murkwater', 'Wasteland')) && move.category === 'Physical') ||
	...
	if (attacker.hasAbility('Dark Aura')) {
            if (field.hasTerrain('Darkness 1')) {
                bpMods.push(5737);
            }
            else if (field.hasTerrain('Darkness 2')) {
                bpMods.push(6144);
            }
			else if (field.hasTerrain('Darkness 3')) {
                bpMods.push(6762);
            }
            else {
                bpMods.push(5448);
	...
	if (attacker.hasAbility('Fairy Aura')) {
		if (field.hasTerrain('Darkness 2')) {
			bpMods.push(4918);
            }
		else if (field.hasTerrain('Darkness 3')) {
			bpMods.push(4508);
    ...
    if ((attacker.hasAbility('Sheer Force') && move.secondaries && !move.isMax) ||
        (attacker.hasAbility('Sand Force') &&
            (field.hasWeather('Sand') || field.hasTerrain('Desert', 'Ashen Beach')) && move.hasType('Rock', 'Ground', 'Steel')) ||
...
function calculateAtModsSMSS(gen, attacker, defender, move, field, desc) {
    ...
    else if ((attacker.hasAbility('Water Bubble') && move.hasType('Water')) || (attacker.hasAbility('Huge Power', 'Pure Power') && move.category === 'Physical' || attacker.hasAbility('Pure Power') && move.category === 'Special' && field.hasTerrain('Psychic'))) {
        atMods.push(8192);
        desc.attackerAbility = attacker.ability;
    }
...
function calculateDfModsSMSS(gen, attacker, defender, move, field, desc, isCritical, hitsPhysical) {
    ...
    else if (defender.hasAbility('Grass Pelt') && field.hasTerrain('Grassy', 'Forest') && hitsPhysical) {
        dfMods.push(6144);
        desc.defenderAbility = defender.ability;
    }
    ...
    if ((defender.hasItem('Eviolite') && ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe)) || (defender.hasItem('Pikanium Z') && ((_b = gen.species.get((0, util_1.toID)(defender.name))) === null || _b === void 0 ? void 0 : _b.nfe)) || (defender.hasItem('Light Ball') && ((_c = gen.species.get((0, util_1.toID)(defender.name))) === null || _c === void 0 ? void 0 : _c.nfe)) || (!hitsPhysical && defender.hasItem('Assault Vest'))) {
        dfMods.push(6144);
        desc.defenderItem = defender.item;
    }
...
function calculateFinalModsSMSS(gen, attacker, defender, move, field, desc, isCritical, typeEffectiveness) {
    ...
    if (field.hasTerrain('Darkness 2') && defender.hasType('Dark')) {
        finalMods.push(3688);
        desc.defenderAbility = defender.ability;
	}
	if (field.hasTerrain('Darkness 2') && defender.hasType('Ghost')) {
        finalMods.push(3688);
        desc.defenderAbility = defender.ability;
	}
	if (field.hasTerrain('Darkness 3') && defender.hasType('Dark')) {
    ...
    if (defender.hasAbility('Fluffy') && move.flags.contact && !attacker.hasAbility('Long Reach')) {
		if (field.hasTerrain('Cloud')) {
			finalMods.push(3072);
			desc.defenderAbility = defender.ability;
		}
		else finalMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    ...
    if (defender.hasAbility('Shadow Shield')) {
        if (field.hasTerrain('Dark Crystal', 'Starlight', 'New World', 'Darkness 1', 'Darkness 2', 'Darkness 3')) {
    ...
    if (move.hasType((0, items_1.getBerryResistType)(defender.item)) && (typeEffectiveness > 1 || move.hasType('Normal')) && !attacker.hasAbility('Unnerve', 'As One (Glastrier)', 'As One (Spectrier)')) {
    ...
    if (attacker.hasAbility('Corrosion') && field.hasTerrain('Corrosive', 'Corrosive Mist')) {
        finalMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    return finalMods;
